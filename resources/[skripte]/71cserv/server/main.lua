...
RegisterCommand("dajmarkere", function(source, args, raw)
	...
	if xPlayer.getGroup()=="superadmin" or xPlayer.getGroup()=="admin" or xPlayer.getGroup()=="owner" then 

		if args[1] and GetPlayerName(args[1]) ~= nil and tonumber(args[2]) then
			TriggerEvent('esx_communityservice:sendToCommunityService', tonumber(args[1]), tonumber(args[2]))	
			markeri('71 Development Logovi » Zatvori', "- " .. GetPlayerName(source) .." - je stavio na markere igraca - " .. GetPlayerName(args[1]) .. " - Kolicina: " .. tonumber(args[2]))
		else
			TriggerClientEvent("bfunkcije:notif", source, "INFO", "Ukucajte /dajmarkere ID kolicina", 5, true)
		end
	else
		TriggerClientEvent("bfunkcije:notif", source, "INFO", "Nemas permisije", 5, false)
	end
end)
	...
	if xPlayer.getGroup()=="superadmin" or xPlayer.getGroup()=="admin" or xPlayer.getGroup()=="owner" then
		if args[1] then
			if GetPlayerName(args[1]) ~= nil then
				...
				TriggerClientEvent("bfunkcije:notif", source, "INFO", "Niste unijeli validan ID", 5, false)
			end
		else
...
AddEventHandler('esx_communityservice:completeService', function()
	...
	MySQL.query('SELECT * FROM communityservice WHERE identifier = @identifier', {
		['@identifier'] = identifier
	}, function(result)

		if result[1] then
			MySQL.execute('UPDATE communityservice SET actions_remaining = actions_remaining - 1 WHERE identifier = @identifier', {
				['@identifier'] = identifier
			})
		else
...
AddEventHandler('esx_communityservice:extendService', function()
	...
	MySQL.query('SELECT * FROM communityservice WHERE identifier = @identifier', {
		...
		if result[1] then
			MySQL.execute('UPDATE communityservice SET actions_remaining = actions_remaining + @extension_value WHERE identifier = @identifier', {
				['@identifier'] = identifier,
...
AddEventHandler('esx_communityservice:sendToCommunityService', function(target, actions_count)
	...
	MySQL.query('SELECT * FROM communityservice WHERE identifier = @identifier', {
		...
		if result[1] then
			MySQL.execute('UPDATE communityservice SET actions_remaining = @actions_remaining WHERE identifier = @identifier', {
				['@identifier'] = identifier,
				['@actions_remaining'] = actions_count
			})
		else
			MySQL.execute('INSERT INTO communityservice (identifier, actions_remaining) VALUES (@identifier, @actions_remaining)', {
				['@identifier'] = identifier,
				['@actions_remaining'] = actions_count
			})
		end
...
function releaseFromCommunityService(target)
	...
	MySQL.query('SELECT * FROM communityservice WHERE identifier = @identifier', {
		...
		if result[1] then
			MySQL.execute('DELETE from communityservice WHERE identifier = @identifier', {
				['@identifier'] = identifier
			})
