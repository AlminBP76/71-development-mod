...
Citizen.CreateThread(function()
	for k,v in ipairs(Config.BarberShops) do
		...
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentSubstringPlayerName('Frizerski Salon')
		EndTextCommandSetBlipName(blip)
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInClothingShop and not hasAlreadyEnteredMarker) or (isInClothingShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Pritisni [E] da promjenis Odjecu")
		end
		...
		if (isInBarberShop and not hasAlreadyEnteredMarker) or (isInBarberShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Pritisni [E] da pristupis frizerskom")
		end
...
RegisterNetEvent('fivem-appearance:saveOutfit', function()
    if Config.UseNewNHKeyboard then
        local keyboard, name = exports["zf_dialog"]:DialogInput({
        ...
        local keyboard = exports["zf_dialog"]:DialogInput({
            header = "Daj ime Outfitu", 
            rows = {
                {
                    id = 0, 
