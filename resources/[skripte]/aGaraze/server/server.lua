RegisterNetEvent('aGaraze:ThrowError', function(text)
    error(text)
end)
...
ESX.RegisterServerCallback('aGaraze:GetVehicles', function(source, callback, type, job)
...
ESX.RegisterServerCallback('aGaraze:GetImpound', function(source, callback, type)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
    local vehicles = {}
...
ESX.RegisterServerCallback('aGaraze:CheckOwnership', function(source, callback, plate, model, job)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
...
RegisterNetEvent('aGaraze:ChangeStored')
AddEventHandler('aGaraze:ChangeStored', function(plate, stored, garage)
...
RegisterNetEvent('aGaraze:SaveVehicle')
AddEventHandler('aGaraze:SaveVehicle', function(vehicle, health, plate, ent)
...
ESX.RegisterServerCallback('aGaraze:PayImpound', function(source, callback, amount)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer then
...
RegisterNetEvent('aGaraze:SpawnVehicle', function(model, plate, coords, heading)
    ...
    CreateThread(function()
        ...
        MySQL.Async.fetchAll('SELECT vehicle, plate, health FROM `owned_vehicles` WHERE plate = @plate', {['@plate'] = ESX.Math.Trim(plate)}, function(result)
            if result[1] then TriggerClientEvent('aGaraze:SetVehicleMods', entityOwner, netId, result[1]) end
        end)
    end)
end)
