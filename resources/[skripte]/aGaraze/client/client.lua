...
function VehicleSpawn(data, spawn)
    TriggerServerEvent('aGaraze:SpawnVehicle', data.vehicle.model, data.vehicle.plate, vector3(spawn.x, spawn.y, spawn.z-1), type(spawn) == 'vector4' and spawn.w or spawn.h)
    if data.type == 'impound' then TriggerServerEvent('aGaraze:PayImpound', data.price) end
end
...
Citizen.CreateThread(function()
    for k, v in pairs(Config.Garages) do
        ...
        exports['qtarget']:AddTargetModel({v.ped or Config.DefaultGaragePed}, {
            options = {
                {
                    event = "aGaraze:GetOwnedVehicles",
...
Citizen.CreateThread(function()
    ...
    exports['qtarget']:AddTargetModel(impoundPeds, {
        options = {
            {
                event = 'aGaraze:GetImpoundedVehicles',
                icon = "fas fa-key",
...
RegisterNetEvent('aGaraze:SetVehicleMods', function(netId, svData)
    ...
    TriggerServerEvent('aGaraze:ChangeStored', svData.plate, false)
    DoVehicleDamage(vehicle, json.decode(svData.health))
end)

RegisterNetEvent('aGaraze:GetImpoundedVehicles')
AddEventHandler('aGaraze:GetImpoundedVehicles', function()
    ESX.TriggerServerCallback('aGaraze:GetImpound', function(vehicles)
        local menu = {}
        ...
        if vehicles ~= nil then
            for k, v in pairs(vehicles) do
                ...
                table.insert(menu, {
                    ...
                    params = {
                        event = 'aGaraze:ImpoundVehicleMenu',
...
RegisterNetEvent('aGaraze:GetOwnedVehicles')
AddEventHandler('aGaraze:GetOwnedVehicles', function()
    ESX.TriggerServerCallback('aGaraze:GetVehicles', function(vehicles)
        local menu = {}
...
RegisterNetEvent('aGaraze:ImpoundVehicleMenu')
AddEventHandler('aGaraze:ImpoundVehicleMenu', function(data)
    TriggerEvent('nh-context:sendMenu', {
        {
            id = 0,
...
RegisterNetEvent('aGaraze:VehicleMenu')
AddEventHandler('aGaraze:VehicleMenu', function(data)
    TriggerEvent('nh-context:sendMenu', {
        {
            id = 0,
            header = Locale('menu_go_back'),
...
RegisterNetEvent('aGaraze:RequestVehicle')
AddEventHandler('aGaraze:RequestVehicle', function(data)
    local spawn = nil
    ...
    for i = 1, #spawn do
        if ESX.Game.IsSpawnPointClear(vector3(spawn[i].x, spawn[i].y, spawn[i].z), 1.0) then
            if data.type == 'impound' then
                ESX.TriggerServerCallback('aGaraze:PayImpound', function(canAfford)
...
RegisterNetEvent('aGaraze:StoreVehicle')
AddEventHandler('aGaraze:StoreVehicle', function(target)
    local health = {}
    local brokenParts = {
        windows = {},
        tires = {},
    ...
    ESX.TriggerServerCallback('aGaraze:CheckOwnership', function(doesOwn)
        if doesOwn then
            if type(doesOwn) == 'table' then return ESX.ShowNotification("You can't store this vehicle here") end

            TriggerServerEvent('aGaraze:ChangeStored', vehPlate, true, currentGarage.zone.name)

            TriggerServerEvent('aGaraze:SaveVehicle', vehProps, health, vehPlate, VehToNet(vehicle))
        else
