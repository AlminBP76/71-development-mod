    ...
    if (event.data.type == "updateVehicleInfos") {
        ...
        $('#contentVehicle').append(`
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Upravljanje</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Max Brzina</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Ubrzanje</span>
                        <div class="bar">
                ...
                if (isEnableTestDrive) {
                    $('#actionButton').append(`
                        <a class="tdButtton" onclick="testDrive()"> Test Voznja </a>
                    `);
                }
    }
...
function menuVehicle(event) {
    ...
    $.post('https://71Salon/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://71Salon/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function openModalMenu(){    
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-content">
            <p class="title">Potvrda o Kupovini:</p>
            <p class="vehicle">Vozilo</p>         
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://71Salon/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://71Salon/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://71Salon/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://71Salon/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://71Salon/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));


                if (dataCar[7].outerText < 1) {
                    document.getElementById("buttonbuy").innerHTML = '<span class="sold-out-text">RASPRODATO</span>';
                }
            }
        break;
    }
            ...
            if (data[i].qtd < 1) {
                ...
                <div class="item-cars"> 
                    <div class="col-lg-3 col-md-6 "> 
                        ...
                        <div class="soldOut">RASPRODATO</div>                 
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://71Salon/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://71Salon/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
