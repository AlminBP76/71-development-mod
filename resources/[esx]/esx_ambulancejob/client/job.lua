...
function OpenVehicleSpawnerMenuHitna(hospital, partNum)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'vozila_meni',
        {
			...
			align    = 'top-right',
            elements = {
				{label = 'Hitna pomoc Auto| 🚑', value = 'dodgeEMS'},--primjer dodavanja vozila
            	{label = 'Bolničarski Kombi | 🚑', value = 'ambulance'},
				{label = 'Hitna pomoc Đip | 🚑', value = 'suburban'},
            }
        },
        function(data, menu)
            if data.current.value == 'ambulance' then
				ESX.Game.SpawnVehicle("ambulance", vector3(293.6851, -583.561, 43.186), 355.86, function(vehicle)
			 ...
			 elseif data.current.value == 'dodgeEMS' then
			 	ESX.Game.SpawnVehicle("dodgeEMS", vector3(293.6851, -583.561, 43.186), 355.86, function(vehicle)
			 		TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
			 	end)
			 	Wait(200)
			 ...
			 elseif data.current.value == 'suburban' then
				ESX.Game.SpawnVehicle("suburban", vector3(293.6851, -583.561, 43.186), 355.86, function(vehicle)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
				end)
				Wait(200)
				local vehicle = GetVehiclePedIsIn(playerPed, false) ---> primjer dodavanja vozila
